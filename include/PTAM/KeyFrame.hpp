...
// Measurment; Each individual Level contains an image, corner points, and
// special corner points which are promoted to Candidate status (the mapmaker
// tries to make new map points from those.)
//
...
// the tracker uses this struct as well: every incoming frame is turned into a
// keyframe before tracking; most of these are then simply discarded, but
// sometimes they're then just added to the map.
struct KeyFrame {
  /**
   * @brief
   */
  ...
  bool bFixed; // Is the coordinate frame of this keyframe fixed? (only true for
  // first KF!)
  ...
  /**
   * @brief This takes an image and calculates pyramid
   * levels etc to fill the keyframe data structures
   * with everything that's needed by the tracker..
   *
   * @param im
   */
  ...
  /**
   * @brief ... while this calculates the rest of the data
