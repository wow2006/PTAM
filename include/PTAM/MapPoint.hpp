...
struct MapPoint {
  /** 
  * @brief Constructor inserts sensible defaults and zeros pointers.
  */
  MapPoint() {
    bBad = false;
    pTData = NULL;
  ...
  Vector<3> v3Center_NC; // Unit vector in Source-KF coords pointing at the
                         // patch center
  Vector<3>
      v3OneDownFromCenter_NC; // Unit vector in Source-KF coords pointing
                              // towards one pixel down of the patch center
  Vector<3>
      v3OneRightFromCenter_NC; // Unit vector in Source-KF coords pointing
  ...
  /** 
  * @brief Calculates above two vectors
  */
