...
/**
 * @brief Each MapPoint has an associated MapMakerData class
...
struct MapMakerData {
  ...
  int GoodMeasCount() { return sMeasurementKFs.size(); }
};

/**
 * @brief MapMaker dervives from CVD::Thread, so everything in void run() is its
 * own thread.
 */
class MapMaker {
public:
   ...
   */
  bool
  InitFromStereo(KeyFrame &kFirst, KeyFrame &kSecond,
   ...
   * @return
   */
  int QueueSize() const { return mvpKeyFrameQueue.size(); }
...
protected:
  ...
  /**
   * @brief Same as the tracker's camera: N.B. not a reference variable!
   */
  ATANCamera mCamera;

  /**
   * @brief The MapMaker thread code lives here
   */
  void run();

  /**
   * @name Functions for starting the map from scratch:
   * @{ */
  ...
  /**
   * @brief Returns point in ref frame B
   *
   * @param se3AfromB
   * @param v2A
   * @param v2B
  ...
  /**
   * @name Bundle adjustment functions:
   * @{ */
  ...
  /**
   * @name Data association functions:
   * @{ */
  ...
  /**
   * @name General Maintenance/Utility:
   * @{ */
  void Reset();
  ...
  /**
   * @brief The above normalized against scene depth, this controls keyframe
   * separation
   */
  ...
  /**
   * @name Thread interaction signalling stuff
   * @{ */
