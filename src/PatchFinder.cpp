...
// bottom.. If this causes problems, just do #define CVD_HAVE_XMMINTRIN 0
...
int PatchFinder::CalcSearchLevelAndWarpMatrix(MapPoint &p, SE3<> se3CFromW,
  ...
  // is the level at which it has been calculated. Vary the search level until
  // the at that level would be appropriate (does not actually modify the
  // matrix.)
  ...
  // reflected, or zero area.. reject these!
  if (dDet > 3 || dDet < 0.25) {
    mbTemplateBad = true;
    return -1;
  } else
...
// generates the template all in one call.
...
void PatchFinder::MakeTemplateCoarseCont(MapPoint &p) {
  ...
  // substantially the same as was made last time. This saves time when the
  // camera is not moving. For this, check that (a) this patchfinder is still
  // working on the same map point and (b) the warping matrix has not changed
  // much.
  ...
  if (bNeedToRefreshTemplate) {
    int nOutside; // Use CVD::transform to warp the patch according the the
                  // warping matrix m2 This returns the number of pixels outside
                  // the source image hit, which should be zero.
    ...
    // re-calculate the patch next time round.
...
// don't really know what the warping matrix should be. (Although to be fair, I
// should do rotation for epipolar, which we could approximate without knowing
// patch depth!)
...
// points which are within radius nRange of the center. (Params are supplied in
// Level0 coords.) Returns true on patch found.
...
// composition used here operates on three variables: x offet, y offset, and
// difference in patch means; hence things like mm3HInv are dim 3, but the
// trivial mean jacobian (always unity, for each pixel) is not stored.
...
// positions in the template image to floating point positions in the target
// keyframe. Interpolation is bilinear, and performed manually (rather than
// using CVD::image_interpolate) since this is a special case where the mixing
...
double PatchFinder::IterateSubPix(KeyFrame &kf) {
  ...
  for (ir.y = 1; ir.y < mnPatchSize - 1; ir.y++) {
    pTopLeftPixel =
        &im[::ir(v2Base) + ImageRef(1, ir.y)]; // n.b. the x=1 offset, as with y
    for (ir.x = 1; ir.x < mnPatchSize - 1; ir.x++) {
