...
#include "PTAM/MEstimator.hpp"
#include "PTAM/OpenGL.hpp"
#include "PTAM/PatchFinder.hpp"
#include "PTAM/ShiTomasi.hpp"
#include "PTAM/SmallMatrixOpts.hpp"
#include "PTAM/TrackerData.hpp"
...
// resets propagate from here. It's always called in the Tracker's thread, often
// as a GUI command.
void Tracker::Reset() {
  mbDidCoarse = false;
...
// tracking functions. bDraw tells the tracker wether it should output any GL
// graphics or not (it should not draw, for example, when AR stuff is being
// shown.)
void Tracker::TrackFrame(Image<CVD::byte> &imFrame, bool bDraw) {
  ...
  // struct This does things like generate the image pyramid and find FAST
  // corners
  mCurrentKF.mMeasurements.clear();
...
// from the user to define the first two key-frames. Salient points are tracked
// between the two keyframes using cheap frame-to-frame tracking (which is very
// brittle - quick camera motion will break it.) The salient points are stored
// in a list of `Trail' data structures. What action TrackForInitialMap() takes
// depends on the mnInitialStage enum variable..
  ...
  if (mnInitialStage == TRAIL_TRACKING_NOT_STARTED) {
    if (mbUserPressedSpacebar) // First spacebar = this is the first keyframe
    {
      ...
      mMessageForUser << "Point camera at planar scene and press spacebar to "
                         "start tracking for initial map."
                      << endl;
    return;
  };
  ...
  if (mnInitialStage == TRAIL_TRACKING_STARTED) {
    ...
    if (mbUserPressedSpacebar) {
      ...
      vector<pair<ImageRef, ImageRef>>
          vMatches; // This is the format the mapmaker wants for the stereo
                    // pairs
      ...
      mMessageForUser << "Translate the camera slowly sideways, and press "
                         "spacebar again to perform stereo init."
                      << endl;
  }
}
...
// potentially-visible-set (PVS); Then it tries to find some points of the PVS
// in the image; Then it updates camera pose according to any points found.
...
// intermediate results; class PatchFinder finds a projected MapPoint in the
...
void Tracker::TrackMap() {
  ...
  // to be measured! First, randomly shuffle the individual levels of the PVS.
  for (int i = 0; i < LEVELS; i++)
  ...
  // high-level PV map points. We use the lowest-res two pyramid levels
  // (LEVELS-1 and LEVELS-2), with preference to LEVELS-1.
  if (bTryCoarse &&
  ...
  // important, so do all of these, with sub-pixel refinement.
  {
    int l = LEVELS - 1;
    ...
    for (unsigned int i = 0; i < avPVS[l].size(); i++)
      vIterationSet.push_back(
          avPVS[l][i]); // Again, plonk all searched points onto the (maybe
                        // already populate) vIterationSet.
  };
  ...
  // limit ourselves to 1000, and choose these randomly.
  ...
  for (int iter = 0; iter < 10; iter++) {
    bool bNonLinearIteration; // For a bit of time-saving: don't do full
                              // nonlinear reprojection at every iteration - it
...
void Tracker::UpdateMotionModel() {
  ...
  // depth. This is used to decide if we should use a coarse tracking stage. We
  // can tolerate more translational vel when far away from scene!
