...
void MapMaker::run() {
  //
  //  while (!shouldStop()) // ShouldStop is a CVD::Thread func which return
  //  true if
  ...
  //    // priority Hierarchy. For example, if there's a new key-frame to be
  //    added
  //    // (QueueSize() is >0) then that takes high priority.
  //
  //    CHECK_RESET;
  ...
  //    if (mbBundleConverged_Recent && !mbBundleConverged_Full && QueueSize()
  //    == 0)
  //      BundleAdjustAll();
  //
  //    CHECK_RESET;
