...
void KeyFrame::MakeKeyFrame_Lite(BasicImage<CVD::byte> &im) {
  ...
  // suppression. Things like that which are needed by the mapmaker but not the
  // tracker go in MakeKeyFrame_Rest();
...
void KeyFrame::MakeKeyFrame_Rest() {
  ...
  // map points, creation of the relocaliser's SmallBlurryImage.
  ...
  for (int l = 0; l < LEVELS; l++) {
    ...
    // with a suitably high score as Candidates, i.e. points which the mapmaker
    // will attempt to make new map points out of.
...
// its own to override CVD's reference-counting behaviour.
...
// run before main()
struct LevelHelpersFiller // Code which should be initialised on init goes here;
