...
GLWindowMenu::GLWindowMenu(string sName, string sTitle)
    : msName(std::move(sName)), msTitle(std::move(sTitle)) {
...
void GLWindowMenu::GUICommandCallBack(void *ptr, string sCommand,
                                      string sParams) {
  static_cast<GLWindowMenu *>(ptr)->GUICommandHandler(sCommand, sParams);
}
  ...
  if (sCommand == msName + ".AddMenuSlider") {
    if (vs.size() < 5) {
      cout << "? GLWindowMenu.AddMenuSlider: Need 5/6 params: Target Menu, "
              "Name, gvar_int name, min, max, NextMenu=\"\""
           << endl;
      return;
    }
...
void GLWindowMenu::Render(int nTop, int nHeight, int nWidth, GLWindow2 &glw) {
  ...
  const double dAlpha = 0.8;

  // No Menu selected  - draw little arrow.
  if (msCurrentSubMenu.empty()) {
  ...
  for (auto i = m.mvItems.rbegin(); i != m.mvItems.rend(); i++) {
    switch (i->type) {
    case Button:
    ...
    case Slider: {
      ...
      FillBox(nBase, (int)(nBase + dFrac * (*mgvnMenuItemWidth + 1)), mnMenuTop,
              mnMenuTop + mnMenuHeight);
      glColor4d(0, 1, 1, dAlpha);
      LineBox(nBase, nBase + *mgvnMenuItemWidth + 1, mnMenuTop,
              mnMenuTop + mnMenuHeight);
...
bool GLWindowMenu::HandleClick(int nMouseButton, int state, int x, int y) {
  ...
  // Clicked on menu name .. . go to root.
  if (nButtonNumber == 0) {
    if (msCurrentSubMenu == "Root") {
      msCurrentSubMenu = "";
    } else {
