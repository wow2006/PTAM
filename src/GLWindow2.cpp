...
void GLWindow2::GUICommandHandler(string sCommand, string sParams) {
...
void GLWindow2::SetupViewport() { glViewport(0, 0, size()[0], size()[1]); }
...
void GLWindow2::HandlePendingEvents() { handle_events(*this); }
...
void GLWindow2::on_mouse_down(GLWindow &win, CVD::ImageRef where, int state,
                              int button) {
  bool bHandled = false;
...
pair<Vector<6>, Vector<6>> GLWindow2::GetMousePoseUpdate() {
  auto result = make_pair(mvLeftPoseUpdate, mvMCPoseUpdate);
  mvLeftPoseUpdate = Zeros;
  mvMCPoseUpdate = Zeros;
  return result;
}
