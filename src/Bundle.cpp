...
template <class MEstimator> bool Bundle::Do_LM_Step(bool *pbAbortSignal) {
  // Reset all accumulators to zero
  ClearAccumulators();

  //  Do a LM step according to Hartley and Zisserman Algo A6.4 in MVG 2nd
  //  Edition Actual work starts a bit further down - first we have to work out
  //  the projections and errors for each point, so we can do tukey reweighting
  ...
  for (list<Meas>::iterator itr = mMeasList.begin(); itr != mMeasList.end();
    ...
    // weight The two jacobians, A and B, with sqrt of the tukey weight w; And
    // also weight the error vector v2Epsilon. That makes everything else
    // automatic. Calc the square root of the tukey weight:
    double dWeight =
  ...
  // should be finished now. So we can find V*i (by adding lambda) and then
  // invert. The next bits depend on mdLambda! So loop this next bit until error
  // goes down.
  ...
  while (dNewError > dCurrentError && !mbConverged && !mbHitMaxIterations &&
    ...
    // combinations, of which there are lots. New code which doesn't waste as
    // much time finding i-jk pairs; these are pre-stored in a per-i list.
    for (unsigned int i = 0; i < mvPoints.size(); i++) {
      Point &p = mvPoints[i];
      int nCurrentJ = -1;
      int nJRow = -1;
...
// new coordinates
template <class MEstimator> double Bundle::FindNewError() {
  ofstream ofs;
  double dNewError = 0;
  vector<double> vdErrorSquared;
  for (list<Meas>::iterator itr = mMeasList.begin(); itr != mMeasList.end();
...
Vector<3> Bundle::GetPoint(int n) { return mvPoints.at(n).v3Pos; }

SE3<> Bundle::GetCamera(int n) { return mvCameras.at(n).se3CfW; }
