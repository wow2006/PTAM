...
inline bool IsCorner(Image<CVD::byte> &im, ImageRef ir,
...
double CalibGridCorner::ExpansionPotential() {
  ...
  // needs to find the last one (because it'll have the most accurate guess for
  // where the last one should be) and so on.
  int nMissing = 0;
  for (int i = 0; i < 4; i++)
...
void CalibImage::GuessInitialPose(ATANCamera &Camera) {
  ...
  // coords Use the standard null-space-of-SVD-thing to find 9 homography parms
